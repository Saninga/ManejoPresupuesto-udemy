    ...
    public class RepositorioTransacciones : IRepositorioTransacciones
    {
        ...
        public async Task<IEnumerable<Transaccion>> ObtenerPorCuentaId(ObtenerTransaccionesPorCuenta modelo)
        {
            ...
            return await connection.QueryAsync<Transaccion>(
                @"SELECT Transacciones.Id, Transacciones.Monto, Transacciones.FechaTransaccion, Categorias.Nombre AS Categoria,
        ...
        public async Task<IEnumerable<Transaccion>> ObtenerPorUsuarioId(ParametroObtenerTransaccionesPorUsuario modelo)
        {
            ...
            return await connection.QueryAsync<Transaccion>(
                @"SELECT Transacciones.Id, Transacciones.Monto, Transacciones.FechaTransaccion, Categorias.Nombre AS Categoria,
                  Cuentas.Nombre as Cuenta, Categorias.TipoOperacionId, Nota
                  FROM Transacciones 
                  INNER JOIN Categorias
                  ON Categorias.Id = Transacciones.CategoriaId
                  INNER JOIN Cuentas
        ...
        {
            using var connection = new SqlConnection(connectionString);
            await connection.ExecuteAsync("Transaccion_Actualizar",
                new
                {
                    transaccion.Id,
                    transaccion.FechaTransaccion,
        ...
        public async Task<IEnumerable<ResultadoObtenerPorSemana>> ObtenerPorSemana(ParametroObtenerTransaccionesPorUsuario modelo)
        {
            ...
            return await connection.QueryAsync<ResultadoObtenerPorSemana>(
                @"Select DATEDIFF(d, @fechaInicio, FechaTransaccion) / 7 +1 AS Semana, SUM(Monto) AS Monto, Categorias.TipoOperacionId
                  FROM Transacciones
                  INNER JOIN Categorias ON Categorias.Id = Transacciones.CategoriaId
        ...
        public async Task<IEnumerable<ResultadoObtenerPorMes>> ObtenerPorMes(int usuarioId, int año)
        {
            ...
            return await connection.QueryAsync<ResultadoObtenerPorMes>(
                @"SELECT MONTH(FechaTransaccion) as Mes, SUM(Monto) as Monto, Categorias.TipoOperacionId
                  FROM Transacciones
                  INNER JOIN Categorias ON Categorias.Id = Transacciones.CategoriaId
